...
<head>
    ...
    <title>Sales &amp; Expense Report {{ $from->format('d M Y') }} to {{ $to->format('d M Y') }} - /Namsa Florals</title>
    <link rel="stylesheet" href="{{ asset('vendor/bootstrap-icons/bootstrap-icons.css') }}">
    <style>
        @page { margin: 16mm 14mm; }
        body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #222; padding: 40px; max-width: 900px; margin: 0 auto; }
        .doc-header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #d63384; padding-bottom: 20px; margin-bottom: 24px; gap: 20px; }
        .doc-brand { display: flex; align-items: center; gap: 12px; }
        .doc-brand img { height: 52px; width: auto; object-fit: contain; }
        .doc-brand .bi-flower1 { font-size: 2.4rem; color: #d63384; }
        ...
        .summary-card.profit { border-left-color: #d63384; }
...
<body>
    ...
    <div class="summary-row">
        <div class="summary-card revenue">
            <div class="label">Revenue ({{ $orderCount }} orders)</div>
        ...
        <div class="summary-card">
            ...
            <div class="value">N$ {{ number_format($avgOrderValue, 2) }}</div>
        </div>
    </div>
        ...
        <tbody>
            @forelse($orders as $o)
                <tr>
                    ...
                    <td class="text-end">N$ {{ number_format((float) $o->total_amount, 2) }}</td>
                </tr>
            @empty
                <tr><td colspan="6" class="text-center text-muted py-3">No orders in this period.</td></tr>
            @endforelse
        </tbody>
    ...
    <div class="doc-footer">
        This report was generated from live order and expense records for /Namsa Florals. Cancelled orders are excluded from revenue.
    </div>
