...
@section('title', 'Your Order - Kaleni Catering Services')
...
<style>
    ...
    .empty-cart-card { position: relative; overflow: hidden; padding: clamp(48px, 8vw, 86px) 24px; text-align: center; background: linear-gradient(145deg, #fff 0%, #fdf3ee 100%); border: 1px solid var(--border); border-radius: 26px; box-shadow: 0 18px 55px rgba(41,33,31,.08); }
    .empty-cart-card::after { content: ''; position: absolute; width: 240px; height: 240px; right: -95px; top: -105px; border: 36px solid rgba(104,11,28,.055); border-radius: 50%; }
    .empty-cart-icon { display: grid; width: 74px; height: 74px; margin: 0 auto 20px; place-items: center; color: var(--primary-dark); background: #fff; border: 1px solid var(--border); border-radius: 50%; box-shadow: 0 14px 32px rgba(41,33,31,.1); font-size: 1.8rem; }
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach($cartItems as $item)
                                        <tr>
                                            <td>
                                                <div class="d-flex align-items-center">
                                                    @if($item->menuItem->image)
                                                        <img src="{{ $item->menuItem->image_url }}" alt="{{ $item->menuItem->name }}" style="width: 60px; height: 60px; object-fit: cover; border-radius: 5px; margin-right: 15px;">
                                                    @endif
                                                    <div>
                                            ...
                                            <td>N$ {{ number_format($item->menuItem->price, 2) }}</td>
                                            <td>
                                                <form action="{{ route('cart.update', $item->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" max="{{ $item->menuItem->stock }}" style="width: 70px;" onchange="this.form.submit()">
                                                </form>
                                            </td>
                                            <td>N$ {{ number_format($item->menuItem->price * $item->quantity, 2) }}</td>
                                            <td>
                                                <form action="{{ route('cart.remove', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Remove this item from your order?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger" title="Remove from order">
                            ...
                            @foreach($cartItems as $item)
                                <div class="cart-mobile-item">
                                    <div class="d-flex align-items-start gap-2">
                                        @if($item->menuItem->image)
                                            <img src="{{ $item->menuItem->image_url }}" alt="{{ $item->menuItem->name }}" class="cart-mobile-img">
                                        @endif
                                        <div class="flex-grow-1 min-w-0">
                                            <strong class="d-block">{{ $item->menuItem->name }}</strong>
                                            <span class="text-muted">N$ {{ number_format($item->menuItem->price, 2) }} each</span>
                                            <div class="d-flex align-items-center justify-content-between mt-2 flex-wrap gap-2">
                                                <form action="{{ route('cart.update', $item->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    <input type="number" name="quantity" value="{{ $item->quantity }}" min="1" max="{{ $item->menuItem->stock }}" class="cart-mobile-qty" onchange="this.form.submit()">
                                                </form>
                                                <span class="fw-bold">N$ {{ number_format($item->menuItem->price * $item->quantity, 2) }}</span>
                                                <form action="{{ route('cart.remove', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Remove this item from your order?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-danger" aria-label="Remove" title="Remove from order"><i class="bi bi-trash"></i></button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <form action="{{ route('cart.clear') }}" method="POST" class="mt-3" onsubmit="return confirm('Clear your entire order? All items will be removed. This cannot be undone.');">
                            @csrf
                            <button type="submit" class="btn btn-outline-danger">Clear Order</button>
                        </form>
                    </div>
                </div>
            </div>
